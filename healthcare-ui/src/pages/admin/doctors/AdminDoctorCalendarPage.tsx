...
export function AdminDoctorCalendarPage() {
  const { id: practitionerId } = useParams<{ id: string }>();
  ...
  const addMutation = useMutation({
    mutationFn: (data: SlotForm) => portalApi.addSlot(practitionerId!, data),
    onSuccess: () => { toast.success('Slot added'); qc.invalidateQueries({ queryKey: ['slots'] }); setAddOpen(false); },
  ...
  const blockMutation = useMutation({
    mutationFn: ({ id, type }: { id: string; type: string }) => portalApi.blockSlot(id, type),
    onSuccess: () => { toast.success('Slot blocked'); qc.invalidateQueries({ queryKey: ['slots'] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  const { register, handleSubmit, formState: { errors } } = useForm<SlotForm>({
  ...
  return (
    <PortalPageWrapper title="Doctor Calendar">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => setAddOpen(true)} className="gap-2 bg-violet-600 hover:bg-violet-700">
        ...
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
              ...
              return (
                <div key={day} className="min-h-20 border rounded p-1 bg-white hover:bg-gray-50">
                  <p className="text-xs font-medium text-gray-600 mb-1">{day}</p>
                  {daySlots.map(s => (
                    <div key={s.id} className={cn('text-xs rounded px-1 py-0.5 mb-0.5 flex items-center justify-between gap-1',
                      ...
                      <div className="flex gap-0.5 flex-shrink-0">
                        {s.available && (
                          <button title="Block" onClick={() => blockMutation.mutate({ id: s.id, type: 'blocked' })}>
                            <Lock className="w-3 h-3" />
                          </button>
                        )}
                        <button title="Delete" onClick={() => deleteMutation.mutate(s.id)}>
